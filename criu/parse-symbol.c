...
long get_symbol_address(const char* path_to_bin, const char* searched_symbol)
{
	...
	memmove(&elf_hdr, bin_cnt, sizeof(elf_hdr));
	if (memcmp(elf_hdr.e_ident, elf_magics, sizeof(elf_magics)) != 0) {
		pr_err("Target is not an ELF executable\n");
		goto err;
	}
	if (elf_hdr.e_ident[EI_CLASS] != ELFCLASS64) {
		pr_err("Only ELF-64 are supported.\n");
		goto err;
	}
#if 0
	if (elf_hdr.e_machine != EM_X86_64) {
		pr_err("Sorry, only x86-64 is supported.\n");
		goto err;
	}
#endif
	...
	for (uint16_t i = 0; i < elf_hdr.e_shnum; i++) {
		...
		memmove(&shdr, cbytes + offset, sizeof(shdr));
		switch (shdr.sh_type) {
			case SHT_SYMTAB:
